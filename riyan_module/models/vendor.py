...
class VendorProduct(models.Model):
	...
	vendor_email = fields.Char(string = "Vendor E-mail", required = True)
	pro_category = fields.Char(string = "Product Category", required = True)
	pro_price = fields.Float(string = "Sellable Price",required = True)
	...
	def send_email_template(self):
		...
		try:
		    template_obj = ir_model_data.get_object_reference('riyan_module', 'req_email_template_id')[1]
		except ValueError:
			template_obj = False
		try:
		    compose_form_obj = ir_model_data.get_object_reference('mail', 'email_compose_message_wizard_form')[1]
		except ValueError:
